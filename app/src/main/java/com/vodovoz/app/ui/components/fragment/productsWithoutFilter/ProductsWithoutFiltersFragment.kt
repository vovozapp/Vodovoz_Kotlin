...
import com.vodovoz.app.ui.components.adapter.categoryTabsAdapter.CategoryTabsAdapter
import com.vodovoz.app.ui.components.adapter.categoryTabsAdapter.CategoryTabsMarginDecoration
...
class ProductsWithoutFiltersFragment : FetchStateBaseFragment() {
    ...
    private lateinit var viewModel: ProductsWithoutFiltersViewModel
    ...
    private fun initViewModel() {
        ...
        )[ProductsWithoutFiltersViewModel::class.java]
    }

    private fun getArgs() {
        viewModel.updateArgs(ProductsWithoutFiltersFragmentArgs.fromBundle(requireArguments()).dataSource)
    }
    ...
    private fun initOnProductClick() {
        onProductClickSubject.subscribeBy { productId ->
            findNavController().navigate(ProductsWithoutFiltersFragmentDirections.actionProductsWithoutFiltersFragmentToProductDetailFragment(productId))
        }
    }
    ...
    private fun updatePager() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.updateProducts().collectLatest { productList ->
                Log.i(LogSettings.ID_LOG, "OBSERVE")
    ...
    private fun showBottomSortSettings() = findNavController().navigate(
        ProductsWithoutFiltersFragmentDirections.actionProductsWithoutFiltersFragmentToSortSettingsBottomFragment2(viewModel.sortType.name)
    )

    private fun showMiniCatalog() = findNavController().navigate(
        ProductsWithoutFiltersFragmentDirections.actionProductsWithoutFiltersFragmentToMiniCatalogBottomFragment(
