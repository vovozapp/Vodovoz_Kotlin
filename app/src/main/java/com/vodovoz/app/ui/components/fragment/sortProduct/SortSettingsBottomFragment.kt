package com.vodovoz.app.ui.components.fragment.sortProduct
...
class SortSettingsBottomFragment : BottomSheetDialogFragment() {
    ...
    private fun getArgs() {
        sortType = SortType.valueOf(SortSettingsBottomFragmentArgs.fromBundle(requireArguments()).sortType)
    }
    ...
    private fun selectSort(sortType: SortType) {
        findNavController().previousBackStackEntry?.savedStateHandle?.set(ProductsFragment.SORT_TYPE, sortType.name)
        dialog?.dismiss()
    }
